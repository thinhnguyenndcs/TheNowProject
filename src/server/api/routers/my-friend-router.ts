...
const userMutualFriendCount = (db: Database, friendUserId: number) => {
  return db
    .selectFrom('friendships as aFriends')
    .innerJoin(
      db
        .selectFrom('friendships')
        ...
        .as('bFriends'),
      'bFriends.friendUserId',
      'aFriends.friendUserId'
    )
    .where('aFriends.status', '=', FriendshipStatusSchema.Values['accepted'])
    .select((eb) => [
      'aFriends.userId as id',
      eb.fn.countAll().as('mutualFriendCount'),
    ])
    .groupBy(['aFriends.userId', 'bFriends.userId', 'aFriends.friendUserId'])
}
