...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       */
      // Check if the friendship with the opposite user has already exists
      const friendShip = await ctx.db
        ...
        .limit(1)
        .execute()
      //Update friendship request record with the opposite user if it already exists
      if (friendShip.length > 0) {
        ...
        //Otherwise, create a new friendship request record with the opposite user
        await ctx.db
  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        // Check if the friendship with the opposite user as the friend has already exists
        const friendShip = await t
          .selectFrom('friendships')
          .where('friendships.userId', '=', ctx.session.userId)
          .where('friendships.friendUserId', '=', input.friendUserId)
          .select('friendships.id')
        ...
        //Update friendship request record with the opposite user as the friend if it already exists
        if (friendShip.length > 0) {
          await t
            .updateTable('friendships')
            .set({ status: FriendshipStatusSchema.Values['accepted'] })
          ...
          // Otherwise, create a new friendship request record with the opposite user as the friend
          await t
            .insertInto('friendships')
            .values({
              userId: ctx.session.userId,
